...
export function createExtensionTools(bus: EventBus, registry: ToolRegistry): Tool[] {
  return [
    {
      name: 'create_tool',
...
`;
        ...
        try {
          ...
          try { unlinkSync(filePath); } catch {}
          return `Tool file compiled but failed to load: ${(err as Error).message}`;
        }
      },
    },
