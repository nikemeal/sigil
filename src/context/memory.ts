...
export class MemoryStore {
  ...
  store(content: string, type: Memory['type'] = 'fact', tags: string[] = []): number {
   ...
   */
  searchByKeyword(query: string, limit: number = 5): MemorySearchResult[] {
    ...
    try {
      const rows = this.db.prepare(`
        ...
        ORDER BY rank
        LIMIT ?
      `).all(terms, MIN_RELEVANCE, limit) as Array<Record<string, unknown>>;
   ...
   */
  searchByVector(queryVector: number[], limit: number = 5): MemorySearchResult[] {
    const rows = this.db.prepare(`
      SELECT m.*, e.vector, e.dimensions
      FROM embeddings e
      JOIN memories m ON m.id = e.memory_id
      WHERE m.relevance > ?
    `).all(MIN_RELEVANCE) as Array<Record<string, unknown>>;

    if (rows.length === 0) return [];

    // Calculate cosine similarity for each
    const scored = rows.map((row) => {
      const storedVector = blobToVector(row.vector as Buffer, row.dimensions as number);
   ...
   */
  search(query: string, queryVector: number[] | null, limit: number = 5): MemorySearchResult[] {
    ...
    const seen = new Map<number, MemorySearchResult>();
  ...
  applyDecay(): void {
    ...
    `).all(MIN_RELEVANCE) as Array<{ id: number; relevance: number; last_accessed: string }>;
  ...
  getAll(): Memory[] {
    ...
    return (rows as Array<Record<string, unknown>>).map((r) => this.rowToMemory(r));
  }
  ...
  private calculateScore(row: Record<string, unknown>): number {
    const rank = Math.abs(row.rank as number || 0);
    const relevance = row.relevance as number;
...
function blobToVector(blob: Buffer, dimensions: number): number[] {
