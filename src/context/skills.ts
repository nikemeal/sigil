...
export class SkillLoader {
  ...
  private enabledList: string[] | undefined;
  ...
  loadAll(): Skill[] {
    ...
    for (const file of files) {
      ...
      if (this.enabledList && !this.enabledList.includes(skill.name)) continue;
   ...
   */
  match(message: string, maxResults = 3): Skill[] {
  ...
  private parseTriggers(raw: unknown): string[] {
    if (!raw) return [];
    let items: string[];
   ...
   */
  private scoreSkill(skill: Skill, messageTokens: Set<string>): number {
    let score = 0;

    // Trigger keywords — 2x weight
    for (const trigger of skill.triggers) {
      if (messageTokens.has(trigger)) score += 2;
    }
