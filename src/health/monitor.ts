...
export class HealthMonitor {
  ...
  private componentState = new Map<string, ComponentStatus>();
  private lastAlertTime = new Map<string, number>();
  private lastResults: HealthCheckResult[] = [];
  private unsubscribers: Array<() => void> = [];

  constructor(bus: EventBus, pool: ProviderPool, db: Database.Database, dbPath: string, intervalMs = 60_000) {
    this.bus = bus;
    this.pool = pool;
    this.db = db;
    this.dbPath = dbPath;
  ...
  start(): void {
    ...
    this.unsubscribers.push(
      this.bus.on('transport:connected', ({ type, id }) => {
        this.updateState(`transport:${type}`, 'healthy', `Connected (${id})`);
      }),
    );

    this.unsubscribers.push(
      this.bus.on('transport:disconnected', ({ type }) => {
    ...
    this.unsubscribers.push(
      this.bus.on('system:error', ({ component }) => {
        if (component) {
    ...
    setTimeout(() => {
      ...
      this.checkAll().catch((err) =>
        console.error('[Health] Initial check failed:', (err as Error).message),
      );
    }, INITIAL_DELAY_MS);
    ...
    this.timer = setInterval(() => {
      if (!this.running) return;
      this.checkAll().catch((err) =>
        console.error('[Health] Periodic check failed:', (err as Error).message),
      );
    }, this.intervalMs);
  ...
  async checkAll(): Promise<HealthCheckResult[]> {
    ...
    if (unhealthy.length > 0 || degraded.length > 0) {
      console.log(
        `[Health] ${unhealthy.length} unhealthy, ${degraded.length} degraded, ` +
  ...
  private updateState(component: string, newStatus: ComponentStatus, message?: string): void {
