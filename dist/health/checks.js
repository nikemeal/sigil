...
export async function checkProviders(pool) {
    ...
    const checks = names.map(async (name) => {
        ...
        try {
            const available = await Promise.race([
                ...
                new Promise((resolve) => setTimeout(() => resolve(false), PROVIDER_TIMEOUT_MS)),
            ]);
            return {
                component: `provider:${name}`,
    ...
    return results.map((r) => r.status === 'fulfilled'
        ? r.value
        : { component: 'provider:unknown', status: 'unhealthy', message: 'Check failed', checkedAt: new Date() });
}
...
export function checkDiskSpace(dbPath) {
    try {
        ...
        let status = 'healthy';
        if (freeBytes < DISK_UNHEALTHY_BYTES)
            status = 'unhealthy';
        else if (freeBytes < DISK_DEGRADED_BYTES)
            status = 'degraded';
        return {
...
export function checkSystemMemory() {
    ...
    if (ratio < MEM_UNHEALTHY_RATIO)
        status = 'unhealthy';
    else if (ratio < MEM_DEGRADED_RATIO)
        status = 'degraded';
    return {
