...
export async function loadModule(modulePath) {
    ...
    if (existsSync(localTsPath)) {
        return await import(pathToFileURL(localTsPath).href);
    }
    if (existsSync(localJsPath)) {
        return await import(pathToFileURL(localJsPath).href);
    }
    ...
    if (existsSync(srcTsPath)) {
        return await import(pathToFileURL(srcTsPath).href);
    }
    if (existsSync(srcJsPath)) {
        return await import(pathToFileURL(srcJsPath).href);
    }
 ...
 */
export function hasLocalOverride(modulePath) {
    const localTsPath = resolve(PROJECT_ROOT, 'local', 'src', `${modulePath}.ts`);
    const localJsPath = resolve(PROJECT_ROOT, 'local', 'src', `${modulePath}.js`);
 ...
 */
export function listLocalOverrides() {
