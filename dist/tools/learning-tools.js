 ...
 */
export function createLearningTools(bus, store) {
    return [
        {
            name: 'reflect',
            description: 'Store a reusable technique you learned from completing a task. Use after finishing something where a specific approach worked well and would apply to future similar tasks.',
            parameters: {
                type: 'object',
                properties: {
                    pattern: {
        ...
        },
        {
            ...
            async execute(_args) {
                ...
                if (all.length === 0)
                    return 'No techniques stored yet.';
                return all
                    .map((t, i) => `${i + 1}. [${t.source}] (used ${t.usageCount}x) id:${t.id.slice(0, 8)}\n   Pattern: ${t.pattern}\n   Technique: ${t.technique}${t.outcome ? `\n   Outcome: ${t.outcome}` : ''}`)
                    .join('\n\n');
            },
        },
        {
