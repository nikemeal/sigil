...
export class TaskRunner {
    ...
    planner;
    config;
    constructor(bus, pool, context, tools, costTracker, store, planner, config) {
        this.bus = bus;
        this.pool = pool;
    ...
    async run(task) {
        ...
        try {
            ...
            for (let i = 0; i < plannedSteps.length; i++) {
                ...
                const step = this.store.addStep(task.id, i + 1, planned.description, model.name, model.tier);
                steps.push(step);
            }
    ...
    async executeStep(task, step, previousResults, totalSteps) {
        ...
        if (!model)
            throw new Error(`Model '${step.assignedModel}' not found`);
        ...
        this.store.completeStep(step.id, completion.content, completion.usage.inputTokens, completion.usage.outputTokens, cost);
        // Track cost in main usage table too
        if (this.costTracker) {
            this.costTracker.log(task.id, model, completion.usage, 'background', 'orchestrated', null);
        }
        const completedStep = {
            ...step,
            status: 'complete',
    ...
    buildStepPrompt(task, step, previousResults, totalSteps) {
