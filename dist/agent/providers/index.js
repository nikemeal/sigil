...
export function createProvider(config) {
    ...
    switch (modelConfig.provider) {
        ...
        default:
            throw new Error(`[Provider] Unknown provider type '${modelConfig.provider}' ` +
...
function resolveDefaultModel(config) {
    if (config.models.length === 0) {
        throw new Error('[Provider] No models configured. Run onboarding or add a [[models]] section to sigil.toml.');
    }
    ...
    if (!model) {
        throw new Error(`[Provider] Default model '${config.defaultModel}' not found in model pool. ` +
            `Available: ${config.models.map((m) => m.name).join(', ')}`);
    }
    return model;
}
/** Create an Anthropic provider, resolving the API key from environment */
function createAnthropicProvider(model) {
    const envVar = model.apiKeyEnv ?? 'ANTHROPIC_API_KEY';
    const apiKey = process.env[envVar];
    if (!apiKey) {
        throw new Error(`[Provider] Anthropic API key not found. Set ${envVar} in your environment or .env file.`);
    }
    return new AnthropicProvider(apiKey);
}
/** Create an OpenAI-compatible provider */
function createOpenAICompatibleProvider(model) {
    if (!model.baseUrl) {
        throw new Error(`[Provider] OpenAI-compatible model '${model.name}' requires a base_url in config.`);
    }
