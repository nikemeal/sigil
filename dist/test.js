...
async function run() {
    ...
    await test('Core types import', async () => {
        ...
        if (!types)
            throw new Error('Failed to import types');
    });
    ...
    await test('Config loads defaults when no file exists', async () => {
        ...
        if (!config.identity.name)
            throw new Error('No identity name');
        if (!config.transports.web.port)
            throw new Error('No web port');
        if (!config.memory.dbPath)
            throw new Error('No memory dbPath');
    });
    await test('Module loader reports no local overrides', async () => {
        ...
        if (overrides.length !== 0)
            throw new Error(`Expected 0 overrides, got ${overrides.length}`);
    });
    ...
    await test('SQLite database initialises with tables', async () => {
        ...
        if (!tableNames.includes('memories'))
            throw new Error('Missing memories table');
        if (!tableNames.includes('embeddings'))
            throw new Error('Missing embeddings table');
        if (!tableNames.includes('summaries'))
            throw new Error('Missing summaries table');
        closeDatabase();
    });
    ...
    `);
        ...
        if (results.length === 0)
            throw new Error('No results for "birthday"');
        if (!results[0].memory.content.includes('May 15th'))
            throw new Error('Wrong memory returned');
        const workResults = store.searchByKeyword('Acme');
        if (workResults.length === 0)
            throw new Error('No results for "Acme"');
        db.close();
    });
    ...
    `);
        ...
        if (results.length === 0)
            throw new Error('No results from combined search');
        db.close();
    });
    ...
    `);
        ...
        if (recent.length !== 3)
            throw new Error(`Expected 3 messages, got ${recent.length}`);
        if (recent[0].content !== 'Hello')
            throw new Error('Wrong message order');
        if (recent[2].content !== 'How are you?')
            throw new Error('Wrong last message');
        db.close();
    });
    ...
    await test('Living profile: create and read', async () => {
        ...
        if (!content.includes('User Profile'))
            throw new Error('Profile missing default content');
        profile.update('# Test Profile\n\nName: Mike');
        if (!profile.get().includes('Mike'))
    ...
    `);
        ...
        if (!results[0].memory.content.includes('Cats'))
            throw new Error('Wrong memory from vector search');
        db.close();
    });
    ...
    `);
        ...
        const message = { id: 'test', content: 'What is my name?', source: 'tui', timestamp: new Date() };
        const llmMessages = await engine.buildContext(message);
        // Should have: system prompt, history (2 msgs), current message
        if (llmMessages.length < 4)
            throw new Error(`Expected at least 4 messages, got ${llmMessages.length}`);
        // System prompt should include identity
        if (!llmMessages[0].content.includes('Sigil'))
            throw new Error('System prompt missing identity');
        // Should include the memory about Mike's name
        if (!llmMessages[0].content.includes('Mike'))
            throw new Error('System prompt missing recalled memory');
        // Last message should be the current one
        const last = llmMessages[llmMessages.length - 1];
        if (last.content !== 'What is my name?')
            throw new Error('Current message not at end');
        // Cleanup
        const { unlinkSync } = await import('node:fs');
        try {
            unlinkSync('/tmp/sigil-test-profile-ctx.md');
        }
        catch { }
        db.close();
    });
    ...
    await test('Tool registry: get definitions excludes denied tools', async () => {
        ...
        if (defs.length !== 1)
            throw new Error(`Expected 1 definition, got ${defs.length}`);
        if (defs[0].name !== 'allowed_tool')
            throw new Error('Wrong tool in definitions');
    });
    ...
    await test('Tool registry: execute denied tool returns error', async () => {
        ...
        if (!result.includes('disabled'))
            throw new Error(`Expected disabled message, got: ${result}`);
    });
    await test('Tool registry: execute unknown tool returns error', async () => {
        ...
        if (!result.includes('Unknown tool'))
            throw new Error(`Expected unknown tool message, got: ${result}`);
    });
    await test('Tool registry: emits tool events', async () => {
        ...
        if (!events.includes('calling'))
            throw new Error('Missing calling event');
    ...
    `);
        ...
        if (!storeResult.includes('Stored'))
            throw new Error(`Remember failed: ${storeResult}`);
        // Recall it
        const recallResult = await recallTool.execute({ query: 'birthday' });
        if (!recallResult.includes('May 15th'))
            throw new Error(`Recall failed: ${recallResult}`);
        // Cleanup
        const { unlinkSync } = await import('node:fs');
        try {
            unlinkSync('/tmp/sigil-test-profile-tools.md');
        }
        catch { }
        db.close();
    });
    ...
    `);
        ...
        const content = profile.get();
        if (!content.includes('Mike'))
            throw new Error('Profile not updated');
        if (!content.includes('cats'))
            throw new Error('Profile missing content');
        const { unlinkSync } = await import('node:fs');
        try {
    ...
    await test('Classifier: casual chat detected', async () => {
        ...
        if (classification.type !== 'chat')
            throw new Error(`Expected chat, got ${classification.type}`);
    });
    await test('Classifier: tool request detected', async () => {
        ...
        if (classification.type !== 'tool')
            throw new Error(`Expected tool, got ${classification.type}`);
    });
    await test('Classifier: complex request detected', async () => {
        ...
        if (classification.type !== 'complex')
            throw new Error(`Expected complex, got ${classification.type}`);
    });
    await test('Classifier: question detected', async () => {
        ...
        if (classification.type !== 'question')
            throw new Error(`Expected question, got ${classification.type}`);
    });
    await test('Classifier: /local override extracted', async () => {
        const { classify } = await import('./router/classifier.js');
        const { classification, cleanMessage } = classify('/local tell me a joke');
        if (classification.override !== 'local')
            throw new Error(`Expected local override, got ${classification.override}`);
        if (cleanMessage !== 'tell me a joke')
            throw new Error(`Message not cleaned: ${cleanMessage}`);
    });
    await test('Classifier: /cloud override extracted', async () => {
        ...
        if (classification.override !== 'cloud')
            throw new Error(`Expected cloud override`);
        if (cleanMessage !== 'explain quantum computing')
            throw new Error(`Message not cleaned`);
    });
    await test('Classifier: /private override extracted', async () => {
        const { classify } = await import('./router/classifier.js');
        const { classification } = classify('/private what is my salary?');
        if (classification.override !== 'private')
            throw new Error(`Expected private override`);
    });
    await test('Router: single model returns it for everything', async () => {
        ...
        const config = {
            version: '2.0.0',
            identity: { name: 'test', personality: 'test' },
            models: [{ name: 'local', provider: 'openai-compatible', model: 'qwen3:8b', tier: 'basic', costPer1kInput: 0, costPer1kOutput: 0, useFor: ['general'] }],
            defaultModel: 'local',
            memory: { dbPath: ':memory:', maxRecallResults: 5 },
            transports: { tui: { enabled: false }, web: { enabled: false, port: 3033, host: '127.0.0.1' }, telegram: { enabled: false } },
            skills: { path: 'skills' },
        ...
        if (chat.model.name !== 'local')
            throw new Error('Wrong model for chat');
        if (complex.model.name !== 'local')
            throw new Error('Wrong model for complex');
    });
    await test('Router: multi-model routes by tier', async () => {
        ...
        const config = {
            ...
            identity: { name: 'test', personality: 'test' },
            models: [
                { name: 'local', provider: 'openai-compatible', model: 'qwen3:8b', tier: 'basic', costPer1kInput: 0, costPer1kOutput: 0, useFor: ['general'] },
                { name: 'claude', provider: 'anthropic', model: 'claude-sonnet', tier: 'standard', costPer1kInput: 0.003, costPer1kOutput: 0.015, useFor: ['general'] },
            ],
            defaultModel: 'local',
            memory: { dbPath: ':memory:', maxRecallResults: 5 },
            transports: { tui: { enabled: false }, web: { enabled: false, port: 3033, host: '127.0.0.1' }, telegram: { enabled: false } },
        ...
        const chat = router.route('hi');
        if (chat.model.name !== 'local')
            throw new Error(`Chat should route to local, got ${chat.model.name}`);
        const tool = router.route('run uptime command');
        if (tool.model.name !== 'claude')
            throw new Error(`Tool should route to claude (standard), got ${tool.model.name}`);
    });
    await test('Router: /local override forces local model', async () => {
        ...
        if (result.model.name !== 'local')
            throw new Error(`Override should force local, got ${result.model.name}`);
    });
    await test('Context trimmer: chat gets fewer messages', async () => {
        ...
        if (chatTrim.maxHistory >= complexTrim.maxHistory)
            throw new Error('Chat should get less history than complex');
        if (!chatTrim.includeTools)
            throw new Error('Chat should include tools');
        if (!complexTrim.includeTools)
            throw new Error('Complex should include tools');
    });
    ...
    `);
        ...
        if (today.totalRequests !== 1)
            throw new Error(`Expected 1 request, got ${today.totalRequests}`);
        if (today.totalCost === 0)
            throw new Error('Cost should be > 0');
        const total = tracker.getTotalSpend();
        if (total === 0)
            throw new Error('Total spend should be > 0');
        db.close();
    });
    // ── Module 9: Diagnosis ──────────────────────────────────────────
    console.log(chalk.dim('\n  Module 9: Diagnosis'));
    await test('read_source reads existing source file', async () => {
        ...
        if (!result.includes('SigilConfig'))
            throw new Error('Should contain SigilConfig type');
        if (!result.includes('No local override'))
            throw new Error('Should report no override');
    });
    await test('read_source returns error for missing file', async () => {
        ...
        if (!result.includes('not found'))
            throw new Error('Should report file not found');
    });
    await test('list_overrides returns empty when none exist', async () => {
        ...
        if (!result.includes('No local overrides'))
            throw new Error('Should report no overrides');
    });
    await test('remove_override handles missing override gracefully', async () => {
        ...
        if (!result.includes('No override found'))
            throw new Error('Should report no override found');
    });
    ...
    await test('list_custom_tools returns empty when none exist', async () => {
        ...
        if (!result.includes('No custom tools'))
            throw new Error('Should report no custom tools');
    });
    ...
    await test('TechniqueResult type is exported', async () => {
        ...
        if (!t.id)
            throw new Error('TechniqueResult not valid');
    });
    ...
    `);
        const tables = db.prepare(`SELECT name FROM sqlite_master WHERE type='table'`).all();
        const tableNames = tables.map((t) => t.name);
        if (!tableNames.includes('techniques'))
            throw new Error('Missing techniques table');
        db.close();
    });
    ...
    `);
        ...
        if (all.length !== 1)
            throw new Error(`Expected 1, got ${all.length}`);
        if (all[0].pattern !== 'summarisation tasks')
            throw new Error('Wrong pattern');
        if (all[0].source !== 'explicit')
    ...
    `);
        ...
        if (results.length === 0)
            throw new Error('No results for "data analysis"');
        if (!results[0].pattern.includes('data analysis'))
            throw new Error('Wrong technique returned');
        db.close();
    });
    ...
    `);
        ...
        if (all[0].usageCount !== 1)
            throw new Error(`Expected usageCount 1, got ${all[0].usageCount}`);
        if (!all[0].lastUsed)
            throw new Error('lastUsed not set');
        db.close();
    });
    ...
    `);
        ...
        const all = store.list();
        if (all.length !== 0)
            throw new Error(`Expected 0, got ${all.length}`);
        db.close();
    });
    ...
    `);
        ...
        if (!evaluator)
            throw new Error('Evaluator failed to instantiate');
        ...
        const all = store.list();
        if (all.length !== 0)
            throw new Error(`Expected 0 techniques, got ${all.length}`);
        db.close();
    });
    ...
    `);
        ...
        if (!reflectTool)
            throw new Error('reflect tool not found');
    ...
    `);
        ...
        if (!listTool)
            throw new Error('list_techniques tool not found');
        const result = await listTool.execute({});
        if (!result.includes('testing'))
            throw new Error(`Expected "testing" in result: ${result}`);
        db.close();
    });
    ...
    `);
        ...
        const message = { id: 'test', content: 'summarise this document', source: 'tui', timestamp: new Date() };
    ...
    await test('Learning: full flow — reflect then inject into context', async () => {
        const db = new Database(':memory:');
        db.exec(`
      CREATE TABLE memories (id INTEGER PRIMARY KEY AUTOINCREMENT, type TEXT NOT NULL DEFAULT 'fact', content TEXT NOT NULL, tags TEXT, relevance REAL NOT NULL DEFAULT 1.0, access_count INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL, last_accessed TEXT NOT NULL);
      CREATE VIRTUAL TABLE memories_fts USING fts5(content, tags, content='memories', content_rowid='id');
    ...
    `);
        ...
        const store = new TechniqueStore(db);
        const tools = createLearningTools(bus, store);
        const engine = new ContextEngine(config, new MemoryStore(db), new ConversationStore(db), new Profile('/tmp/test-learning-full.md'), null, null, bus, store);
        // Step 1: agent explicitly stores a technique via reflect tool
        const reflectTool = tools.find((t) => t.name === 'reflect');
        await reflectTool.execute({
            pattern: 'parsing tasks',
            technique: 'Always validate encoding before parsing to avoid garbled output',
        });
        // Step 2: on a message about parsing, the technique appears in the system prompt
        const message = { id: 'msg-1', content: 'parsing this config file', source: 'tui', timestamp: new Date() };
        const context = await engine.buildContext(message);
        const systemPrompt = context[0].content;
        if (!systemPrompt.includes('Techniques from past experience')) {
            throw new Error('Technique section missing from system prompt');
        }
        if (!systemPrompt.includes('parsing tasks')) {
            throw new Error('Technique pattern missing from system prompt');
        }
        // Step 3: usageCount was incremented
        const all = store.list();
        if (all.length !== 1)
            throw new Error(`Expected 1 technique, got ${all.length}`);
        if (all[0].usageCount !== 1)
            throw new Error(`Expected usageCount 1, got ${all[0].usageCount}`);
        db.close();
    });
    ...
    await test('Update config: loads defaults when [update] section absent', async () => {
        ...
        if (config.update.enabled !== true)
            throw new Error(`Expected update.enabled=true, got ${config.update.enabled}`);
        if (config.update.checkInterval !== '24h')
            throw new Error(`Expected checkInterval='24h', got '${config.update.checkInterval}'`);
        if (config.update.remoteBranch !== 'origin/main')
            throw new Error(`Expected remoteBranch='origin/main', got '${config.update.remoteBranch}'`);
    });
    ...
    await test('parseDuration: throws on invalid format', async () => {
        ...
        if (!threw)
            throw new Error('Expected parseDuration to throw on "1week"');
    });
    await test('OverrideReviewer: no-op when local/ directory is missing', async () => {
        ...
        await reviewer.review(bus, null);
        if (events.length !== 0)
            throw new Error(`Expected no events, got ${events.join(', ')}`);
    ...
    await test('OverrideReviewer: removes override when src file not found', async () => {
        ...
        await reviewer.review(bus, null);
        if (removed.length !== 1)
            throw new Error(`Expected 1 removed override, got ${removed.length}`);
        if (removed[0] !== 'tools/custom.js')
            throw new Error(`Expected "tools/custom.js", got "${removed[0]}"`);
    ...
    await test('Update tools: check_for_updates and apply_update created', async () => {
        ...
        const tools = createUpdateTools(bus, checker, updater, null);
        if (tools.length !== 2)
            throw new Error(`Expected 2 tools, got ${tools.length}`);
        const names = tools.map((t) => t.name);
        if (!names.includes('check_for_updates'))
            throw new Error('Missing check_for_updates tool');
        if (!names.includes('apply_update'))
            throw new Error('Missing apply_update tool');
        if (!tools.every((t) => t.approval === 'auto'))
            throw new Error('Tools should be auto-approved');
    });
    // ── Summary ───────────────────────────────────────────────────────
