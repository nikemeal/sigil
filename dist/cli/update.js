...
export async function runUpdateCli(checkOnly) {
    loadEnv();
    ...
    try {
        ...
        console.error('[Sigil] Update check failed:', err.message);
        process.exit(1);
    }
    if (!result.hasUpdate) {
    ...
    console.log(`[Sigil] Update available: ${result.commitCount} commit(s) behind ` +
    ...
    try {
        ...
        console.error('[Sigil] Update failed:', err.message);
        process.exit(1);
    }
}
