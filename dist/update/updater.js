...
export class Updater {
    ...
    remoteBranch;
    applying = false;
    constructor(cwd, remoteBranch) {
        this.cwd = cwd;
        this.remoteBranch = remoteBranch;
    }
    async apply(bus, pool) {
        if (this.applying) {
        ...
        try {
            ...
            if (slashIndex === -1) {
                throw new Error(`Invalid remoteBranch format: "${this.remoteBranch}". Expected "remote/branch" (e.g. "origin/main").`);
            }
            ...
            console.log(`[Updater] Update complete: ${previousSha.slice(0, 7)} → ${newSha.slice(0, 7)}. Restarting...`);
